...
test("renderer includes canonical Link pages in the primary navigation", async () => {
  ...
  assert.ok(navItemsSource.indexOf('id: "skills"') < navItemsSource.indexOf('id: "wiki"'));
  assert.ok(navItemsSource.indexOf('id: "wiki"') < navItemsSource.indexOf('id: "apps"'));
...
test("Wiki owns the tools and apps catalog", async () => {
  ...
  assert.match(wikiToolbarSource, /<select value=\{sort\}[\s\S]*?<TableRefreshButton onClick=\{refreshActiveWikiPage\} busy=\{wikiRefreshing\} label=\{wikiRefreshLabel\} \/>/);
  ...
  assert.match(skillsTabSource, /<span role="columnheader">Skill<\/span>\s*<span role="columnheader">Owner<\/span>\s*<span role="columnheader" aria-label="Open skill" \/>/);
  ...
  assert.match(styles, /\.wikiToolbar\s*\{[\s\S]*?grid-template-columns:\s*44px minmax\(260px,\s*1fr\) 112px 44px/);
  assert.match(styles, /@media \(max-width:\s*1120px\)\s*\{[\s\S]*?\.wikiToolbar\s*\{[\s\S]*?grid-template-columns:\s*44px minmax\(0,\s*1fr\) 112px 44px/);
...
test("app marketplace uses the managed Link App Publisher catalog", async () => {
  ...
  const wikiViewSource = app.slice(app.indexOf("function WikiView"), app.indexOf("function SettingsView"));
...
test("page layouts use full-width app canvas", async () => {
  ...
  assert.match(styles, /\.pageSectionSidebarHeading\s*\{[\s\S]*?min-height:\s*var\(--top-control-height\)/);
...
test("settings uses account phone and tools tabs with nested sections", async () => {
  const app = await readFile("src/renderer/App.tsx", "utf8");
  const api = await readFile("src/renderer/api.ts", "utf8");
  const main = await readFile("src/main/main.js", "utf8");
  const preload = await readFile("src/main/preload.cjs", "utf8");
  const styles = await readFile("src/renderer/styles.css", "utf8");
  const settingsViewSource = app.slice(app.indexOf("function SettingsView"), app.indexOf("function CredentialSection"));
  ...
  assert.match(app, /title: "PHONE"/);
  assert.match(app, /title: "TOOLS"/);
  assert.doesNotMatch(app, /title: "TELEPHONY"/);
  assert.doesNotMatch(settingsViewSource, /title: "LINK APP"/);
  assert.match(app, /\["auth", "Auth", ShieldCheck\]/);
  assert.match(app, /\["plugins", "Plugins", Link2\]/);
  assert.match(app, /\["agentmail", "Email", Mail\]/);
  assert.match(app, /\["domains", "Domains", Globe\]/);
  ...
  assert.doesNotMatch(settingsViewSource, /\["agents", "Agents", Bot\]/);
  assert.doesNotMatch(app, /\["speak", "Speech", Mic\]/);
  assert.doesNotMatch(app, /tab === "speak"/);
  assert.doesNotMatch(app, /<ScribesView embedded \/>/);
  assert.match(app, /function ScribesView/);
  assert.match(app, /\["speech", "Speech", SlidersHorizontal\]/);
  assert.match(app, /activeTab === "speech"\) return <SpeakSettingsPanel \/>/);
  assert.doesNotMatch(app, /<h1>Scribes<\/h1>/);
  assert.doesNotMatch(app, /<PageSectionHeader[\s\S]*?parent="Scribes"/);
  assert.match(app, /<PageSectionHeader[\s\S]*?parent=""[\s\S]*?title=\{activeLabel\}/);
  assert.match(app, /heading="Scribes"/);
  const scribesTtsPanelSource = app.slice(app.indexOf("function ScribesTtsPanel"), app.indexOf("function ScribesMeetingNotesPanel"));
  assert.doesNotMatch(scribesTtsPanelSource, /<SpeakSettingsPanel \/>/);
  ...
  assert.match(app, /\["dialer", "Dialer", Grid3X3\]/);
  const accountSettingsGroupSource = settingsViewSource.slice(settingsViewSource.indexOf('title: "ACCOUNT"'), settingsViewSource.indexOf('title: "PHONE"'));
  const toolsSettingsGroupSource = settingsViewSource.slice(settingsViewSource.indexOf('title: "TOOLS"'), settingsViewSource.indexOf("] as const}", settingsViewSource.indexOf('title: "TOOLS"')));
  assert.match(accountSettingsGroupSource, /\["auth", "Auth", ShieldCheck\][\s\S]*?\["plugins", "Plugins", Link2\][\s\S]*?\["wiki", "Wiki", BookOpen\]/);
  assert.doesNotMatch(accountSettingsGroupSource, /\["agentmail", "AgentMail", Mail\]/);
  assert.doesNotMatch(accountSettingsGroupSource, /\["design", "App Design", Palette\]/);
  assert.match(toolsSettingsGroupSource, /\["domains", "Domains", Globe\][\s\S]*?\["agentmail", "Email", Mail\][\s\S]*?\["design", "App Design", Palette\]/);
  assert.doesNotMatch(settingsViewSource, /\["design", "Tool Design", Grid2X2\]/);
  assert.doesNotMatch(settingsViewSource, /title: "LINK APP"[\s\S]*?\["design", "App Design", Palette\]/);
  ...
  assert.match(app, /<PageSectionHeader parent="Link" title=\{settingsHeadingTitle\} action=\{settingsHeaderAction\} \/>/);
  assert.match(app, /const settingsHeaderAction = tab === "dialer" \? dialerBuilderActions : tab === "wiki" \? \(/);
  assert.match(app, /New Source/);
  assert.match(styles, /\.settingsView \.pageSectionSidebar\s*\{[\s\S]*?background:\s*#fbfaf9/);
  assert.match(styles, /\.settingsView \.pageSectionSidebarHeading\s*\{[\s\S]*?border-bottom:\s*1px solid var\(--border\);[\s\S]*?background:\s*#fbfaf9/);
  assert.match(styles, /\.settingsView \.pageSectionSidebarHeading \+ \.pageSectionSidebarGroup \.pageSectionSidebarGroupTitle\s*\{[\s\S]*?border-top:\s*0/);
  assert.match(styles, /\.settingsView \.pageSectionSidebarGroupTitle\s*\{[\s\S]*?background:\s*#f1efeb/);
  assert.match(styles, /\.desktop\[data-theme="dark"\] \.settingsView \.pageSectionSidebarGroupTitle\s*\{[\s\S]*?background:\s*#242321/);
  assert.match(styles, /\.settingsView \.pageSectionSidebar button\.selected\s*\{[\s\S]*?box-shadow:\s*inset 0 0 0 1px var\(--border-soft\)/);
  assert.doesNotMatch(app, /aria-label="Setup sections"/);
  assert.doesNotMatch(app, /\["credentials", "Credentials", ShieldCheck\]/);
  for (const label of ["Auth", "Plugins", "AgentMail", "Contacts", "Voice AI", "Numbers", "Dialer", "Speech", "Domains", "App Design", "Dialer Builder"]) {
    assert.match(app, new RegExp(label));
  }
  assert.match(app, /className="chatSessionRows directoryTable settingsDirectoryTable"/);
  assert.match(app, /className="chatSessionRows directoryTable settingsDirectoryTable settingsAuthDirectoryTable"/);
  ...
  assert.match(app, /tab === "agentmail" && renderAgentMailSettingsTab\(\)/);
  const pluginsTabSource = app.slice(app.indexOf("function renderPluginsTab"), app.indexOf("function renderAgentMailSettingsTab"));
  assert.match(pluginsTabSource, /renderSettingsDirectoryToolbar\("Search plugins, tools, sources, or access"/);
  assert.match(pluginsTabSource, /className="directoryNameCell settingsProviderNameCell"/);
  assert.match(pluginsTabSource, /<SettingsConnectorLogo connector=\{connector\} \/>/);
  assert.match(pluginsTabSource, /<span role="columnheader">Type<\/span>/);
  assert.doesNotMatch(pluginsTabSource, /<span role="columnheader">Tools<\/span>/);
  assert.doesNotMatch(pluginsTabSource, /<small>\{connector\.description\}<\/small>/);
  assert.doesNotMatch(pluginsTabSource, /<span role="cell">\{settingsConnectorTools\(connector\)\.length\}<\/span>/);
  assert.match(styles, /\.settingsDirectoryTable \.chatResultRows\s*\{[\s\S]*?align-content:\s*start/);
  assert.match(styles, /\.settingsDirectoryResultRow,[\s\S]*?grid-template-columns:\s*minmax\(240px,\s*1fr\) minmax\(140px,\s*0\.22fr\) minmax\(132px,\s*0\.2fr\) var\(--table-action-column-width\)[\s\S]*?min-height:\s*var\(--table-row-min-height\)/);
  const authTabSource = app.slice(app.indexOf("function renderAuthTab"), app.indexOf("function renderPluginsTab"));
  assert.doesNotMatch(authTabSource, /<span role="columnheader">Type<\/span>/);
  assert.doesNotMatch(authTabSource, /<span role="columnheader">Fields<\/span>/);
  assert.doesNotMatch(authTabSource, /<span role="cell">\{group\.id\}<\/span>/);
  assert.doesNotMatch(authTabSource, /<small>\{credentialHelpCopy\(group\)\}<\/small>/);
  assert.doesNotMatch(authTabSource, /renderSettingsDirectoryToolbar/);
  assert.doesNotMatch(authTabSource, /settingsQuery\.trim\(\)\.toLowerCase\(\)/);
  assert.doesNotMatch(authTabSource, /settingsStatusFilter/);
  assert.match(authTabSource, /const authCredentials = \[\.\.\.requiredCredentials\]\.sort\(compareCredentialGroups\)/);
  assert.match(app, /!\["agent-control-plane", "mcp-proxy", "guru", "pylon"\]\.includes\(group\.id\)/);
  assert.match(app, /className="directoryNameCell settingsProviderNameCell"/);
  assert.match(app, /<SettingsProviderLogo group=\{group\} \/>/);
  assert.match(styles, /\.directoryResultRow > \.settingsProviderNameCell\s*\{[\s\S]*?display:\s*grid/);
  assert.match(styles, /\.settingsProviderNameCell\s*\{[\s\S]*?grid-template-columns:\s*30px minmax\(0,\s*1fr\)/);
  assert.match(styles, /\.settingsProviderLogoBox\s*\{[\s\S]*?width:\s*30px;[\s\S]*?height:\s*30px;[\s\S]*?border:\s*1px solid var\(--border\);[\s\S]*?border-radius:\s*999px/);
  assert.match(styles, /\.settingsProviderLogoBox\.telnyx svg\s*\{[\s\S]*?width:\s*22px/);
  assert.match(styles, /\.settingsAuthDirectoryTable \.settingsDirectoryResultRow,[\s\S]*?grid-template-columns:\s*minmax\(300px,\s*1fr\) minmax\(132px,\s*0\.2fr\) var\(--table-action-column-width\)/);
  assert.doesNotMatch(styles, /\.settingsAuthExpandedDetail\s*\{/);
  assert.match(authTabSource, /if \(selectedGroup\) \{[\s\S]*?return renderAuthCredentialDetail\(selectedGroup, credentialConnected\(selectedGroup\)\)/);
  assert.doesNotMatch(app, /expanded && renderAuthCredentialDetail\(group, connected\)/);
  const settingsSource = settingsViewSource;
  ...
  assert.match(app, /const pageSectionSidebarDefaultWidth = 142/);
  assert.match(app, /const pageSectionSidebarSnapThreshold = 10/);
  assert.match(app, /className="pageSectionSidebarTabs" role="tablist" aria-label=\{label\}/);
  assert.match(app, /className="pageSectionSidebarResizeHandle"[\s\S]*?role="separator"[\s\S]*?onDoubleClick=\{resetSidebarWidth\}[\s\S]*?onPointerDown=\{startSidebarResize\}[\s\S]*?onPointerMove=\{resizeSidebar\}[\s\S]*?onPointerUp=\{stopSidebarResize\}/);
  assert.match(app, /shell\?\.style\.setProperty\("--page-section-sidebar-width", `\$\{clampedWidth\}px`\)/);
  assert.match(app, /shell\?\.style\.removeProperty\("--page-section-sidebar-width"\)/);
  assert.match(styles, /\.pageSectionShell\s*\{[\s\S]*?grid-template-columns:\s*var\(--page-section-sidebar-width,\s*142px\) minmax\(0,\s*1fr\)[\s\S]*?min-height:\s*calc\(100vh - var\(--app-top-inset\) - var\(--app-bottom-inset\)\)/);
  assert.match(styles, /\.pageSectionSidebar\s*\{[\s\S]*?align-self:\s*stretch[\s\S]*?min-height:\s*100%[\s\S]*?border-right:\s*1px solid var\(--border\)/);
  assert.match(styles, /\.pageSectionSidebarTabs\s*\{[\s\S]*?min-height:\s*100%[\s\S]*?padding:\s*0 12px var\(--app-bottom-inset\)/);
  assert.match(styles, /\.pageSectionSidebarHeading\s*\{[\s\S]*?display:\s*grid[\s\S]*?justify-items:\s*center[\s\S]*?gap:\s*8px[\s\S]*?min-height:\s*112px[\s\S]*?background:\s*#fbfaf9[\s\S]*?font-size:\s*24px[\s\S]*?font-weight:\s*760/);
  assert.match(app, /HeadingIcon && <HeadingIcon size=\{24\} \/>/);
  assert.match(styles, /\.pageSectionSidebarHeading svg\s*\{[\s\S]*?width:\s*24px[\s\S]*?height:\s*24px[\s\S]*?color:\s*currentColor/);
  assert.match(styles, /\.pageSectionSidebarGroupTitle\s*\{[\s\S]*?display:\s*flex[\s\S]*?margin:\s*0 -12px 2px[\s\S]*?min-height:\s*var\(--top-control-height\)[\s\S]*?border-top:\s*1px solid var\(--border\)[\s\S]*?border-bottom:\s*1px solid var\(--border\)[\s\S]*?text-align:\s*center/);
  ...
  assert.match(styles, /\.pageSectionSidebarResizeHandle\s*\{[\s\S]*?right:\s*-4px[\s\S]*?width:\s*8px[\s\S]*?cursor:\s*col-resize/);
  ...
  assert.match(app, /type SettingsTab = "auth" \| "plugins" \| "agentmail" \| "contacts" \| "assistants" \| "numbers" \| "dialer" \| "domains" \| "design" \| "wiki"/);
  assert.match(settingsViewSource, /function renderAgentMailSettingsTab/);
  assert.match(settingsViewSource, /Email account/);
  assert.match(settingsViewSource, /AGENTMAIL_API_KEY/);
  assert.match(settingsViewSource, /AGENTMAIL_DOMAIN/);
  assert.match(settingsViewSource, /groups=\{\[agentMailCredential\]\}/);
  assert.match(app, /if \(group\.id === "agentmail"\) return group\.fields\.some\(\(field\) => field\.name === "AGENTMAIL_API_KEY" && field\.configured\)/);
  assert.match(app, /if \(name === "AGENTMAIL_API_KEY"\) return "AgentMail API Key"/);
  assert.match(styles, /\.settingsView \.agentMailSettingsPanel,[\s\S]*?\.settingsView \.wikiSettingsPanel\s*\{/);
  assert.match(styles, /\.agentMailReadiness\s*\{/);
  assert.match(settingsViewSource, /function renderDomainsSettingsTab/);
  assert.match(settingsViewSource, /Tool domains/);
  assert.match(settingsViewSource, /Internal Telnyx domain/);
  assert.match(settingsViewSource, /Custom domain/);
  assert.match(settingsViewSource, /tab === "domains" && renderDomainsSettingsTab\(\)/);
  assert.match(settingsViewSource, /\["wiki", "Wiki", BookOpen\]/);
  assert.match(settingsViewSource, /function renderWikiSettingsTab/);
  const wikiTableSource = settingsViewSource.slice(
    settingsViewSource.indexOf('<section className="wikiSourceTableSection"'),
    settingsViewSource.indexOf("{activeWikiSources.length === 0", settingsViewSource.indexOf('<section className="wikiSourceTableSection"')),
  );
  assert.match(settingsViewSource, /wikiSourceEditorOpen && \(/);
  assert.match(settingsViewSource, /setWikiSourceEditorOpen\(true\)/);
  assert.match(settingsViewSource, /setWikiSourceEditorOpen\(false\)/);
  assert.match(settingsViewSource, /const activeWikiSources = wikiSources\.filter\(\(source\) => source\.enabled\)\.sort/);
  assert.match(settingsViewSource, /aria-label="Active Wiki sources"/);
  assert.match(settingsViewSource, /activeWikiSources\.map\(\(source\)/);
  assert.match(app, /function wikiSourceConnectionType/);
  assert.match(app, /if \(type === "github"\) return "Git"/);
  assert.match(app, /if \(type === "telnyx_support" \|\| type === "telnyx_developers"\) return "API"/);
  assert.match(settingsViewSource, /wikiSourceConnectionType\(source\.type\)/);
  assert.match(settingsViewSource, /setSelectedSettingsDetailId\(expanded \? "" : `wiki:\$\{source\.id\}`\)/);
  assert.match(settingsViewSource, /expanded && renderWikiSourceDetail\(source\)/);
  assert.doesNotMatch(settingsViewSource, /className="accessCard wikiSourceSummary"/);
  assert.doesNotMatch(wikiTableSource, /<span role="columnheader">Target<\/span>/);
  assert.doesNotMatch(wikiTableSource, /<span role="columnheader">Actions<\/span>/);
  assert.doesNotMatch(wikiTableSource, /<small>\{source\.description\}<\/small>/);
  assert.doesNotMatch(wikiTableSource, /className="wikiSourceTargetCell"/);
  assert.doesNotMatch(wikiTableSource, /title="Repoint source"/);
  assert.doesNotMatch(wikiTableSource, /title="Remove source"/);
  assert.match(settingsViewSource, /No active Wiki sources yet\./);
  ...
  assert.match(styles, /\.wikiSourceRow\s*\{[\s\S]*?grid-template-columns:\s*minmax\(260px,\s*1fr\) minmax\(140px,\s*0\.22fr\) minmax\(132px,\s*0\.2fr\) var\(--table-action-column-width\)/);
  assert.match(styles, /\.wikiSourceExpandedDetail\s*\{/);
  ...
  assert.match(main, /new URL\(configured \|\| "\/agents\/new", `\$\{agentControlPlaneUrl\(\)\}\/`\)/);
  assert.match(main, /url\.searchParams\.set\("source", "link-desktop"\)/);
  assert.match(main, /Buffer\.from\(JSON\.stringify\(draft\), "utf8"\)\.toString\("base64url"\)/);
  ...
  assert.match(main, /id:\s*"telnyx", label:\s*"Telnyx"/);
  assert.match(main, /id:\s*"google-workspace", label:\s*"Google"/);
...
test("chat and phone stay available in the persistent assistant panel", async () => {
  ...
  assert.match(styles, /--assistant-popup-edge-inset:\s*var\(--assistant-panel-edge-inset\)/);
  assert.match(styles, /\.assistantRuntimeMenu\s*\{[\s\S]*?right:\s*var\(--assistant-popup-edge-inset\)[\s\S]*?left:\s*var\(--assistant-popup-edge-inset\)/);
...
test("chats page uses compact session table rows and a detail review page", async () => {
  ...
  assert.match(styles, /\.chatResultRowHead > \[role="columnheader"\],[\s\S]*?\.phoneNumberRowHead > \[role="columnheader"\],[\s\S]*?\.chatResultRow > span:not\(\.bulkSelectCell\),[\s\S]*?\.chatSessionNameCell\s*\{[\s\S]*?justify-self:\s*stretch[\s\S]*?text-align:\s*left/);
  assert.match(styles, /\.directoryResultRow > \[role="columnheader"\],[\s\S]*?\.directoryResultRow > span:not\(\.bulkSelectCell\):not\(\.directoryRowActions\)\s*\{[\s\S]*?justify-self:\s*stretch[\s\S]*?text-align:\s*left/);
  ...
  assert.match(app, /className="drivePathBar" aria-label="Current Drive folder"/);
  assert.match(app, /className="driveBreadcrumbs" aria-label="Drive path"/);
...
test("workboard page has provider-aware adapters and local fallback UI", async () => {
  ...
  assert.match(styles, /\.workboardSearchRow\s*\{[\s\S]*?grid-template-columns:\s*44px 44px minmax\(280px,\s*1fr\) 44px/);
  ...
  assert.match(styles, /\.workboardRowCard\s*\{[\s\S]*?grid-template-columns:\s*38px minmax\(220px,\s*1\.1fr\) minmax\(320px,\s*1\.7fr\) minmax\(120px,\s*0\.45fr\) minmax\(82px,\s*0\.24fr\)[\s\S]*?min-height:\s*var\(--table-row-min-height\)[\s\S]*?border-bottom:\s*1px solid var\(--border-soft\)/);
  assert.match(styles, /\.workboardRows\.bulkEditing \.workboardRowCard\s*\{[\s\S]*?grid-template-columns:\s*28px 38px minmax\(220px,\s*1\.1fr\) minmax\(320px,\s*1\.7fr\) minmax\(120px,\s*0\.45fr\) minmax\(82px,\s*0\.24fr\)/);
  ...
  assert.match(styles, /\.workboardRowDescription\s*\{[\s\S]*?text-overflow:\s*ellipsis[\s\S]*?white-space:\s*nowrap/);
  assert.match(styles, /\.workboardRowTime\s*\{[\s\S]*?justify-self:\s*start[\s\S]*?text-overflow:\s*ellipsis/);
  assert.match(styles, /\.workboardRowActions\s*\{[\s\S]*?position:\s*absolute[\s\S]*?opacity:\s*0[\s\S]*?pointer-events:\s*none/);
...
test("phone page links existing Telnyx numbers and keeps calling controls in the sidebar", async () => {
  ...
  assert.match(dialerConfig, /name: "Dialer"/);
  ...
  assert.match(dialerConfig, /enabledFeatures: \["crm", "transcription", "recording", "notes", "salesforce-notes-sync", "dispositions", "call-timer", "analytics"\]/);
  ...
  assert.match(app, /const \[numberSortMode, setNumberSortMode\] = useState<"az" \| "za" \| "type">\("az"\)/);
  ...
  assert.match(styles, /\.phoneNumberSearchRow\s*\{[\s\S]*?grid-template-columns:\s*44px minmax\(280px,\s*1fr\) minmax\(120px,\s*160px\) 44px/);
  assert.match(app, /phoneContactTable phoneContentTable/);
  assert.match(app, /phoneContactSearchRow/);
  assert.match(styles, /\.phoneContactSearchRow\s*\{[\s\S]*?grid-template-columns:\s*44px minmax\(280px,\s*1fr\) minmax\(120px,\s*160px\) 44px/);
  ...
  assert.match(app, /phoneInboxRowHead chatResultRowHead" role="row">\s*<span role="columnheader">From<\/span>\s*<span role="columnheader">Subject<\/span>\s*<span role="columnheader">Date<\/span>\s*<span role="columnheader" aria-label="Open thread" \/>/);
  ...
  assert.match(app, /Link shows unread Gmail threads only\. Use filters to separate direct mail from group-alias mail\./);
  ...
  assert.match(app, /const callRollups = useMemo\(\(\) =>/);
  assert.match(app, /const filteredCallRollups = callRollups\.filter/);
  ...
  assert.match(app, /const selectedCallRecordings = selectedCallDetail\?\.calls\.filter/);
  assert.match(app, /const selectedCallTranscripts = selectedCallDetail\?\.calls\.filter/);
  ...
  assert.match(app, /body=\{loadingCallHistory \? "Pulling recent Telnyx call detail records\." : callQuery\.trim\(\)[\s\S]*?\? "Try another search term or filter\." : "Recent Telnyx account calls will roll up by phone number here after call detail records are available\."\}/);
  ...
  assert.match(app, /<span role="columnheader">Last<\/span>/);
  assert.match(app, /phoneCallRollupSummary\(call\)/);
  ...
  assert.match(app, /contactHeaderActions[\s\S]*?Auth[\s\S]*?Plugins[\s\S]*?Add Contact/);
  assert.match(app, /openSettingsTab=\{setTab\}/);
  assert.match(app, /Search contacts, numbers, sources, or roles/);
  assert.match(app, /contactSortMode/);
  ...
  assert.match(dialerConfig, /name: "Call Timer"/);
  assert.match(dialerConfig, /timer-alert/);
  ...
  assert.match(styles, /\.phoneInboxRow\s*\{[\s\S]*?grid-template-columns:\s*minmax\(140px,\s*220px\) minmax\(260px,\s*1fr\) minmax\(92px,\s*128px\) var\(--table-action-column-width\)/);
  ...
  assert.match(styles, /\.dialerBuilderLayout\s*\{[\s\S]*?column-gap:\s*14px/);
  assert.match(styles, /\.dialerBuilderFeatures\s*\{[\s\S]*?grid-template-rows:\s*auto auto auto[\s\S]*?align-content:\s*start/);
...
test("calendar page uses Google Calendar events for meetings, calls, and descriptions", async () => {
  ...
  assert.match(app, /className="calendarViewToggle"/);
  ...
  assert.match(styles, /\.calendarViewToggle\s*\{/);
  assert.match(styles, /\.calendarViewToggle button\.active\s*\{/);
...
test("agents page includes search and squad filtering for discovered agents", async () => {
  ...
  assert.match(agentsViewSource, /<span role="columnheader">Agent<\/span>\s*<span role="columnheader">Owner<\/span>\s*<span role="columnheader" aria-label="Open agent" \/>/);
  ...
  assert.match(app, /Connected account context/);
  ...
  assert.match(styles, /\.pageSectionShell\s*\{[\s\S]*?grid-template-columns:\s*var\(--page-section-sidebar-width,\s*142px\) minmax\(0,\s*1fr\)[\s\S]*?border:\s*0/);
